    ...
    if ($errors == array())
    {
        ...
        if ($siteDb->type == 2) {
			...
			$siteDb->parameters = array(
				...
				'dont_update_dolibarr_company' => GETPOSTISSET('ecommerce_dont_update_dolibarr_company') ? (GETPOST('ecommerce_dont_update_dolibarr_company', 'int') > 0 ? 1 : 0) : 0,
            );
			if (empty($conf->global->PRODUCT_DISABLE_WEIGHT)) {
...
$ecommerceDontUpdateDolibarrCompany = (isset($siteDb->parameters['dont_update_dolibarr_company']) ? $siteDb->parameters['dont_update_dolibarr_company'] : 0);
